...
6. **`env(safe-area-inset-bottom)` em TODO elemento ancorado ao rodapé da tela** — regra permanente após bug real encontrado em 2026-07-12 e REINCIDENTE no mesmo dia (modal do Parceiro de Responsabilização, `social.js`): o rodapé do modal de Autoavaliação (`.modal-footer`, botão "Próximo") ficava atrás da barra de navegação/gestos do Android, e depois o mesmo aconteceu num modal novo (`#socialBody`) cujos botões de ação foram colocados direto dentro de `.modal-body` em vez de um `.modal-footer` dedicado — provando que a regra pontual (listar cada elemento) não é suficiente, porque depende de lembrar de aplicá-la a cada novo componente.
   - **Correção estrutural aplicada**: `.modal-body` em si agora tem `padding-bottom: calc(16px + env(safe-area-inset-bottom))` no CSS base (`styles.css`), então QUALQUER conteúdo interativo dentro de um `.modal-body` — mesmo renderizado dinamicamente via JS, mesmo sem `.modal-footer` — já fica protegido por padrão, sem exigir que quem escreve a feature nova lembre da regra.
   - Mesmo assim, ao criar um modal novo: **prefira usar `.modal-footer` para botões de ação primários** (fica fixo, fora do scroll do body) em vez de deixá-los soltos no fim do `.modal-body`. Isso é sobre ergonomia (o botão não desaparece scrollado), não só sobre o bug de safe-area (que já está coberto de qualquer forma).
   - Isso vale também para qualquer elemento com `position: fixed` colado no rodapé fora de um modal: `.toast`, FABs (`.install-btn`, `.edit-fab`), e qualquer footer/CTA fixo futuro — sempre checar se `bottom`/`padding-bottom` inclui `calc(Npx + env(safe-area-inset-bottom))`.
   - **Checklist obrigatório antes de considerar qualquer UI nova "pronta"**: testar visualmente (ou ao menos revisar o CSS computado) do elemento mais baixo da tela/modal contra a safe-area — não basta o código "parecer" coberto pela regra geral; a área mais baixa de CADA tela nova precisa ser conferida explicitamente, pois foi exatamente essa checagem que faltou da primeira vez.
   - **Ocorrência 3, mesmo dia**: o bug voltou uma TERCEIRA vez, agora no `body` da própria página principal (`padding-bottom: 88px` fixo, sem `env()` — cobria só a altura dos FABs, não a barra de gestos). Isso não era modal nem toast, era o `<footer>` da tela principal, nunca auditado porque as correções 1 e 2 só olharam para modais. **Regra definitiva**: toda vez que esse bug for reportado — mesmo que pareça já resolvido — rode um grep de `bottom:|padding-bottom:|margin-bottom:` e `position:\s*fixed` no `styles.css` INTEIRO na mesma sessão, não só corrija o elemento reportado. Corrigir só o sintoma já falhou 2 vezes seguidas; auditoria completa do arquivo é a única coisa que resolveu de fato na 3ª vez.
